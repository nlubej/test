...
module.exports = function summary(commonBody, body, previousCommonBody) {
  ...
  if (commonBody?.evaluation?.paymentPlan) {

    summaryData.paymentPlanSummary = componentMappings.getComponentMappingForSummary('PaymentPlanSummaryWithStateAndChangeAmounts')(commonBody, previousCommonBody, previousCommonBody?.summary?.paymentPlanSummary);

    if (summaryData.paymentPlanSummary?.paymentPlanInstallmentsPerItem?.length > 0) {
      const getPremiumByCoverageSummary = componentMappings.getComponentMappingForSummary('PremiumByCoverageSummary');

      summaryData.premiumByCoverageSummary = getPremiumByCoverageSummary({ paymentPlanPerItem: summaryData.paymentPlanSummary.paymentPlanInstallmentsPerItem });
